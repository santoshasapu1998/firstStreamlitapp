...
# Create sliders for numeric features except price_gap_40
st.subheader("Input Numeric Features")
numeric_features = ['carrier_cnt', 'med_ded_std', 'med_moop_std']
numeric_input = {}
...
# Process constant input data
# Add a placeholder for `price_gap_40` in the numeric input
numeric_input['price_gap_40'] = 0  # Placeholder for scaling

# Convert numeric inputs to DataFrame and scale
numeric_df = pd.DataFrame([numeric_input])
numeric_df = pd.DataFrame(scaler.transform(numeric_df), columns=numeric_features + ['price_gap_40'])
...
input_data = pd.concat([numeric_df.drop(columns=['price_gap_40']), encoded_categorical_df], axis=1)

# Generate predictions for different price_gap_40 values
...
for price_gap in price_gap_values:
    ...
    # Update `price_gap_40` dynamically
    scaled_price_gap = scaler.transform(pd.DataFrame({'price_gap_40': [price_gap]}))[:, -1]
    temp_input['price_gap_40'] = scaled_price_gap
    market_share_prediction = model.predict(temp_input)[0]
